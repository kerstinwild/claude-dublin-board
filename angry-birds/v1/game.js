/* Angry Birds — v1
   Canvas + matter.js. Mouse-controlled slingshot, destructible structures, pigs, scoring, stars. */
(() => {
'use strict';
...
function makeBird(type, x, y){
  ...
  b.btype = type; b.r = t.r; b.launched = false; b.dead = false;
  return b;
}
  ...
  if(state === 'fly'){
    ...
    if(flying){
      ...
      if((slow && launchWatch > 900) || off){
        // settle: mark bird spent, load next
...
function drawBird(b){
  ...
  // eyebrows (angry)
  ctx.strokeStyle='#3a0d08'; ctx.lineWidth=r*0.18; ctx.lineCap='round';
...
function drawHUD(){
  ...
  let bx = 40;
  for(const b of remaining){ miniBird(bx, 44); bx += 34; }
  // level label
...
function miniBird(x,y){ ctx.fillStyle='#d62b1f'; ctx.beginPath(); ctx.arc(x,y,12,0,7); ctx.fill(); ctx.fillStyle='#3a0d08'; ctx.strokeStyle='#3a0d08'; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(x-3,y-6); ctx.lineTo(x+6,y-2); ctx.stroke(); ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(x+3,y-2,4,0,7); ctx.fill(); ctx.fillStyle='#222'; ctx.beginPath(); ctx.arc(x+4,y-2,2,0,7); ctx.fill(); ctx.fillStyle='#f5a623'; ctx.beginPath(); ctx.moveTo(x+9,y); ctx.lineTo(x+16,y+2); ctx.lineTo(x+9,y+5); ctx.fill(); }
